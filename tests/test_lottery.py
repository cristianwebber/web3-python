...
import pytest
from scripts.config import network_args, LOCAL_BLOCKCHAIN_ENVIRONMENTS, get_account, fund_with_link, get_contract
...
def test_can_pick_winner_correctly(lottery_contract):
    ...
    lottery_contract.enter(
        {"from": get_account(account_idx=1), "value": lottery_contract.getEntranceFee()}
    )
    lottery_contract.enter(
        {"from": get_account(account_idx=2), "value": lottery_contract.getEntranceFee()}
    )
    fund_with_link(lottery_contract)
